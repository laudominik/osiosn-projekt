...
def _safe_nonzero(p: torch.Tensor) -> int:
    try:
        ...
        # Quantized / packed tensors don't support nonzero(); treat all as non-zero
        return p.numel()
...
def profile(model_module, datamodule) -> dict:
    ...
    Handles quantized models gracefully (no deepcopy, CPU-only).
    """
    ...
    def _measure(device, x):
        # Try deepcopy+move first; fall back to in-place CPU if it fails
        # (quantized models crash on deepcopy due to C++ packed params)
        m = None
        try:
            m = deepcopy(model).to(device).eval()
        except Exception:
            if str(device) != "cpu":
                return None  # GPU measurement needs an independent copy
            m = model
            m.eval()
        xi = x.to(device)
        try:
    ...
    # GPU inference (skipped for quantized models which are CPU-only)
    accel = _best_device()
    if accel is not None and str(accel) != "cpu":
        ...
        if gpu_ms is not None:
            ...
            print(f"  Inference {str(accel).upper():<4} : N/A (not available for this model)")
    else:
        results["inference_gpu_ms"] = None
