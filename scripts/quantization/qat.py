...
from osiosn import WasteSortingDataModule, WasteSortingModule, save_results
...
def _prepare_qat_fx(pl_module: pl.LightningModule) -> bool:
    ...
    pl_module.model = quant_fx.prepare_qat_fx(
        pl_module.model, qconfig_dict, example_inputs
    )
...
for noise_tag, noise_rate, p_dog in NOISE_VARIANTS:
    ...
    os.remove(tmp_zip)
    print(
        f"  Rozmiar: {metrics['size_mb']:.2f} MB  (zip: {metrics['size_zip_mb']:.2f} MB)"
    )
